...
public class UserManager {
	...
	public static void addSocialRequest(
		...
		SocialRequest socialRequest =
			SocialRequestLocalServiceUtil.addRequest(
				user.getUserId(), 0, User.class.getName(), user.getUserId(), 1,
				StringPool.BLANK, receiverUser.getUserId());
	...
	public static String addPortletId(
		...
		portletId =
			layoutTypePortlet.addPortletId(0, portletId, columnId, -1, false);
	...
	public synchronized static User addUser(
		String firstName, long companyId, String themeId)
		throws Exception {
		...
		try {
			user =
				UserLocalServiceUtil.addUser(
					...
					companyId, false, firstName, firstName, false,
					firstName.toLowerCase(), firstName.toLowerCase() +
						"@liferay.com", 0, "", LocaleUtil.getDefault(),
					firstName, "", lastName, 0, 0, true, 8, 15, 1974, job,
					groupIds, new long[] {}, roleIds, new long[] {}, false,
					serviceContext);
		}
		catch (DuplicateUserEmailAddressException ex) {
			user =
				UserLocalServiceUtil.getUserByEmailAddress(
					companyId, firstName.toLowerCase() + "@liferay.com");
		}
	...
	private static void setupUserProfile(
		long companyId, String themeId, Group guestGroup, User user)
		throws Exception {

		Group group = user.getGroup();
		if (themeId != null && !themeId.isEmpty()) {
			LayoutSetLocalServiceUtil.updateLookAndFeel(
				group.getGroupId(), false, themeId, "01", "", false);
		}
		...
		try {
			...
			cloudArticle =
				addArticle(
					"/cloud-structure.xml", "/cloud-template.vm",
					"cloud-article.xml",
					UserLocalServiceUtil.getDefaultUserId(companyId),
					guestGroup.getGroupId(),
					SocialDriverConstants.CLOUD_ARTICLE_ID);
		...
		try {
			...
			assetListArticle =
				addArticle(
					"/assetlist-structure.xml", "/assetlist-template.vm",
		...
		try {
			...
			addArticle(
				"/experts-structure.xml", "/experts-template.vm",
				"experts-article.xml",
				UserLocalServiceUtil.getDefaultUserId(companyId),
				guestGroup.getGroupId(),
				SocialDriverConstants.EXPERTSLIST_ARTICLE_ID);
		...
		addPortletBorder(
			layout, addPortletId(layout, PortletKeys.ACTIVITIES, "column-2"));
		addPortletBorder(
			layout,
		...
		layout =
			addLayout(group, "Expertise", false, "/expertise", "2_columns_ii");
		...
		portletId =
			addPortletId(layout, PortletKeys.JOURNAL_CONTENT, "column-2");
		configureJournalContent(
		...
		addPortletBorder(
			layout, addPortletId(layout, PortletKeys.BLOGS, "column-2"));

		// Workspace layout

		layout =
			addLayout(group, "Workspace", false, "/workspace", "2_columns_i");
	...
	// people
	private static void assignRandomFriends(User user)
		throws Exception {
		...
		for (int i = 0; i < friendCount; i++) {
			...
			User randUser =
				UserLocalServiceUtil.getUsers(frienduser, frienduser + 1).get(0);
	...
	private static void assignAddressTo(User user, ServiceContext serviceContext)
		...
		Country country =
			countrys.get((int) (Math.random() * (double) countrys.size()));
		List<Region> regions =
			RegionServiceUtil.getRegions(country.getCountryId());
		long regionId = 0;
		if (regions != null && !regions.isEmpty()) {
			regionId =
				regions.get((int) (Math.random() * (double) regions.size())).getRegionId();
		}

		AddressLocalServiceUtil.addAddress(
			user.getUserId(), Contact.class.getName(), user.getContactId(),
			street, "", "", city, zip, regionId, country.getCountryId(), 11002,
	...
	public static InputStream getInputStream(String path)
		...
		return UserManager.class.getResourceAsStream("/resources/" + path);
	}
	...
	public static JournalArticle addJournalArticle(
		long userId, long groupId, String articleId, String fileName,
		String structureId, String templateId, ServiceContext serviceContext)
		throws Exception {
		...
		return JournalArticleLocalServiceUtil.addArticle(
			userId, groupId, 0, 0, 0, articleId, false,
			JournalArticleConstants.VERSION_DEFAULT, titleMap, null, content,
			"general", structureId, templateId, StringPool.BLANK, 1, 1, 2008,
			0, 0, 0, 0, 0, 0, 0, true, 0, 0, 0, 0, 0, true, true, false,
			StringPool.BLANK, null, null, StringPool.BLANK, serviceContext);
	}

	public static DDMStructure addJournalStructure(
		long userId, long groupId, String structureId, String fileName)
		throws Exception {
		...
		return DDMStructureLocalServiceUtil.addStructure(
			userId, groupId, StringPool.BLANK,
			PortalUtil.getClassNameId(JournalArticle.class), structureId,
			nameMap, descriptionMap, xsd,
			PropsKeys.JOURNAL_ARTICLE_STORAGE_TYPE,
			DDMStructureConstants.TYPE_DEFAULT, serviceContext);
	}

	public static DDMTemplate addJournalTemplate(
		long userId, long groupId, String templateId, long structureId, String fileName)
		throws Exception {
		...
		return DDMTemplateLocalServiceUtil.addTemplate(
			...
			structureId, templateId, nameMap, descriptionMap,
	...
	public static JournalArticle addArticle(
		String structPath, String tmplPath, String articlePath, long userId,
		...
		DDMTemplate tmpl =
			addJournalTemplate(
				userId, groupId, articleId, struct.getStructureId(), tmplPath);
		ServiceContext serviceContext = new ServiceContext();
		serviceContext.setScopeGroupId(groupId);
		serviceContext.setAddGroupPermissions(true);
		serviceContext.setAddGuestPermissions(true);
		return addJournalArticle(
			userId, groupId, articleId, articlePath, struct.getStructureKey(),
	...
	public static void addPortletBorder(Layout layout, String portletId)
		...
		PortletPreferencesLocalServiceUtil.updatePreferences(
			ownerId, ownerType, layout.getPlid(), portletId, prefs);
	...
	public static byte[] getPortraitBytes()
		throws Exception {

		String id =
			SocialDriverConstants.GRAVATAR_IDS[(int) (Math.random() * (double) SocialDriverConstants.GRAVATAR_IDS.length)];
		String urlS =
			"http://2.gravatar.com/avatar/" + id + "?s=80&d=identicon";
